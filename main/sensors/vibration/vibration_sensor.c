...
esp_err_t vibration_sensor_init(void) {
    ...
    i2c_master_bus_add_device(bus_handle, &dev_cfg, &mpu_dev);

    // Acorda o MPU-6050 escrevendo 0x00 no PWR_MGMT_1 (0x6B)
    uint8_t pwr_cmd[2] = {0x6B, 0x00};
    esp_err_t ret = i2c_master_transmit(mpu_dev, pwr_cmd, sizeof(pwr_cmd), 100);

    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "MPU-6050 inicializado");
    } else {
...
esp_err_t vibration_sensor_read(vibration_sensor_data_t *data) {
    ...
    data->accel_z = (float)((int16_t)(raw[4] << 8 | raw[5])) / MPU6050_ACCEL_FACTOR;
...
char* vibration_sensor_build_json(const vibration_sensor_data_t *data) {
    if (!data){
        ESP_LOGE(TAG, "Dados invalidos para o parser do Json");
        return NULL;
    }
    ...
    if (!root) {
        ESP_LOGE(TAG, "Falha ao alocar memoria para o objeto cJSON");
        return NULL;
    }
    ...
    if (!json) {
        ESP_LOGE(TAG, "Falha ao serializar o objeto cJSON");
        return NULL;
    }
